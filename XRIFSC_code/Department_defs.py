...
class dep_defs(ddepartment):
    def barriers(self, t):
        ...
        def initialize_barrier(index):
            ...
            for vars in ["titleresul","use_ent", "radside", "radforward", "leakvar", "existvar", "sellocation", "forw", "side",
                        "laks", "entk", "leak"]:
                self.d[f"{vars} {index}{room_number}"] = None
            ...
            self.d[f"numbmat {index}{room_number}"] = ttk.Spinbox(master=self.d[f"barrierf {index}{room_number}"],
                from_=1, to=spinbox_max, width=5, textvariable=self.var[f"vnumbmat {index}{room_number}"],
        ...
        while current_barriers > max_barriers:
            ...
            self.destroy_widgets([barrier_frame_key])
            self.destroy_widgets([label_B_key])
            self.destroy_widgets([radpw_key])
            self.destroy_widgets([radsw_key])
            current_barriers -= 1
        ...
        if num_barriers > prev_num_barriers:
            ...
            for i in range(prev_num_barriers, num_barriers):
                ...
                self.d[f"radpw {index}{room_number}{i}"] = ttk.Radiobutton(
                    ...
                    variable=self.var[f"radiob_w {index}{room_number}{i}"], text=f"Primary", value=1,
                ...
                self.d[f"radsw {index}{room_number}{i}"] = ttk.Radiobutton(
                    master=self.d[f"barrierf {index}{room_number}"],
                    variable=self.var[f"radiob_w {index}{room_number}{i}"], text=f"Secondary", value=2,
                    command=lambda e=index, nr=room_number: self.barrier_sel(e, nr, t, i))
    ...
    def barrier_sel(self, e, nr, t, i):
        ...
        self.create_radio_button(key=f"selxray1 {e}{nr}{i}", master=self.d[barrierf_key],
                                 text="X-Ray Room\n(Suggested NCRP 147)", variable=self.var[f"vselxray {e}{nr}{i}"],
        ...
        self.create_radio_button(key=f"selxray2 {e}{nr}{i}", master=self.d[barrierf_key], text="Give kVp",
                                 variable=self.var[f"vselxray {e}{nr}{i}"], value=2, row=i * 10 + 4, column=2,
        ...
        # ===== Selects Secondary =====
            ...
            self.create_radio_button(key=f"airkncrp {e}{nr}{i}", master=self.d[barrierf_key],
                ...
                variable=self.var[f"unairkerv {e}{nr}{i}"], value=1, row=i * 10 + 6, column=0,
    ...
    def XrRoom(self, e, nr, i):
        ...
        if self.var[xray_key].get() == 1:
            # Destroy existing widget if it exists
            self.destroy_widgets([selxroom_key])
            ...
            if self.var[radiob_w_key].get() == 1:
                ...
                self.d[selxroom_key] = ttk.Spinbox(master=self.d[barrierf_key], from_=25, to=150, increment=5,
                                                   textvariable=self.var[vsexroom_key], width=10)
    ...
    def pres(self, e, nr, i):
        ...
        if self.var[preshvar_key].get() == 1:
            ...
            self.d[radbucky_key] = ttk.Radiobutton(master=self.d[f"barrierf {e}{nr}"], variable=self.var[radiob_pre_key],
                                                   text="Bucky", value=1)
            self.d[radbucky_key].grid(row=i * 10 + 6, column=3, pady=6.5, padx=5, sticky="w")
            # Create and place the 'Cross-table' Radiobutton
            self.d[radcross_key] = ttk.Radiobutton(master=self.d[f"barrierf {e}{nr}"], variable=self.var[radiob_pre_key],
        ...
        # If preshielding is deselected (0), destroy the widgets if they exist
            ...
            self.destroy_widgets([radbucky_key])
            self.destroy_widgets([radcross_key])
    ...
    def owork(self, e, nr, i):
        ...
        self.var[workv_key] = IntVar(value=0)  # Store IntVar in dictionary
        # Check if the workload option is selected (1)
        if self.var[work_key].get() == 1:
            # Create the 'Write total Workload' Radiobutton
            self.d[radwork_key] = ttk.Radiobutton(master=self.d[f"barrierf {e}{nr}"], variable=self.var[workv_key],
                                                  text="Write total\nWorkload (mA∙min∙week\u207B\u00b9):", value=1,
                                                  command=lambda: self.workloadbar(e, nr, i))
            # Create the 'Number of Patients' Radiobutton
            self.d[radnumb_key] = ttk.Radiobutton(master=self.d[f"barrierf {e}{nr}"], variable=self.var[workv_key],
        ...
        elif self.var[work_key].get() == 0:
            self.destroy_widgets([radwork_key])
            self.destroy_widgets([radnumb_key])
            self.destroy_widgets([f"numpapwe {e}{nr}{i}"])
            self.destroy_widgets([f"worentry {e}{nr}{i}"])
    ...
    def workloadbar(self, e, nr, i):
        ...
        if workload_type == 1:
            self.destroy_widgets([numpapwebar_key])  # Destroy patient entry if it exists
            self.destroy_widgets([worebar_key])
        ...
        # Handle workload "Number of Patients"
            ...
            self.destroy_widgets([worebar_key])  # Destroy workload entry if it exists
            self.destroy_widgets([numpapwebar_key])
    ...
    def unairk(self, e, nr, i):
        ...
        if self.var[unairkerv_key].get() == 1:
            ...
            self.create_radio_button(key=f"leak {e}{nr}{i}", master=self.d[f"barrierf {e}{nr}"],
                text="Leakage Radiation", variable=self.var[f"airkerv {e}{nr}{i}"], value=1, row=i * 10 + 7, column=0,
            ...
            self.create_radio_button(key=f"side {e}{nr}{i}", master=self.d[f"barrierf {e}{nr}"], text="Side-Scatter",
                variable=self.var[f"airkerv {e}{nr}{i}"], value=2, row=i * 10 + 7, column=1,
                command=lambda: self.leakage(e, nr, i))

            self.create_radio_button(key=f"forw {e}{nr}{i}", master=self.d[f"barrierf {e}{nr}"],
                text="Forward/Backscatter", variable=self.var[f"airkerv {e}{nr}{i}"], value=3, row=i * 10 + 7, column=2,
                command=lambda: self.leakage(e, nr, i))
    ...
    def leakage(self, e, nr, i):
        ...
        if self.var[airkerv_key].get() == 1:  # Leakage and side-scatter
            self.var[radiob_leak_key] = IntVar(value=0)
            self.create_radio_button(key=f"radside {e}{nr}{i}", master=self.d[f"barrierf {e}{nr}"],
                text="Leakage and Side-Scatter", variable=self.var[radiob_leak_key], value=1, row=i * 10 + 8, column=0)
            self.create_radio_button(key=f"radforward {e}{nr}{i}", master=self.d[f"barrierf {e}{nr}"],
                text="Leakage and Forward/Backscatter", variable=self.var[radiob_leak_key], value=2, row=i * 10 + 8,
                column=1)
            self.create_radio_button(key=f"radleak {e}{nr}{i}", master=self.d[f"barrierf {e}{nr}"], text="Only Leakage",
                variable=self.var[radiob_leak_key], value=0, row=i * 10 + 8, column=2)
    ...
    def _add_xray_room_additional_widgets(self, index, room_number, t):
        ...
        self.raoccup = ttk.Radiobutton(
            master=self.d[f"barrierf {index}{room_number}"],
            variable=self.var[f"vraoccup {index}{room_number}"], text="Write occupancy factor (T):",
        ...
        self.raseloccup = ttk.Radiobutton(
            ...
            variable=self.var[f"vraoccup {index}{room_number}"], value=2,
    ...
    def existbarrier(self, e,nr,t):
        if self.var["existvar " + str(e)+nr].get() == 1:
            ...
            self.var["vmaterex "+str(e)+nr] = StringVar()
        ...
        elif self.var["existvar " + str(e)+nr].get() == 0:
            if self.d["existla " + str(e)+nr] is not None:
                self.d["existla " + str(e)+nr].destroy()
                self.d["existmat " + str(e)+nr].destroy()
                self.d["materex " + str(e)+nr].destroy()
